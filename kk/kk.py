...
def reward_func(queries, prompts,labels):
    scores = []
    for qr,prompt,label in zip(queries,prompts,labels):
        hash_str = hashlib.md5(prompt.encode('utf8')).hexdigest()
        # ground_truth_t = ppl3_dataset[hash_str]['reward_model']['ground_truth']
        ground_truth = json.loads(label)
        # assert(ground_truth==ground_truth_t)
        r = compute_score(qr, ground_truth)
        scores.append(r)
